...
class MatchSettingActivity : BasicActivity() {
    private lateinit var binding: ActivityReportBinding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView(this, R.layout.activity_match_setting)
        ...
        binding.etReport.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                if(s.toString().length > 500) {
                    ...
                    binding.dividerReport.dividerColor = ContextCompat.getColor(this@MatchSettingActivity, R.color.error)
                    binding.tvReportLength.setTextColor(ContextCompat.getColor(this@MatchSettingActivity,R.color.error))
                } else {
                    binding.tvReportError.visibility = View.GONE
                    binding.tvReportLength.visibility = View.VISIBLE
                    binding.tvReportLength.setTextColor(ContextCompat.getColor(this@MatchSettingActivity,R.color.gray_09))
                    binding.dividerReport.dividerColor = ContextCompat.getColor(this@MatchSettingActivity, R.color.gray_07)
                }

                if(s.toString().isEmpty())  {
                    binding.tvReportError.visibility = View.GONE
                    binding.btnReportConfirm.setBackgroundResource(R.drawable.button_round_disabled)
                    binding.btnReportConfirm.setTextColor(ContextCompat.getColor(this@MatchSettingActivity, R.color.gray_07))
                    binding.btnReportConfirm.isEnabled = false
                } else {
                    binding.btnReportConfirm.setBackgroundResource(R.drawable.button_round)
                    binding.btnReportConfirm.setTextColor(ContextCompat.getColor(this@MatchSettingActivity, R.color.gray_12))
